...
class KochatApi:
    ...
    def __build(self):
        ...
        def fill_slot(uid: str, text: str) -> dict:
            ...
            :return: json 딕셔너리
            """
            print("fill_slot text",text)
            prep = self.dataset.load_predict(text, self.embed_processor)
            entity = self.entity_recognizer.predict(prep)
            print('fill_slot entity',entity)
            text = self.dataset.prep.tokenize(text, train=False)
            print('fill_slot_text',text)
            intent = self.dialogue_cache[uid]['intent']
            print('fill_slot_intent', intent)

            if entity.count('O')>0:
                 entity.remove('O')

            for i in entity:
                if i in self.dialogue_cache[uid]['entity']:
